...
def rate(x,theta,alpha, phi):
    return phi * numpy.exp(-(x-theta)**2/(2*alpha**2))

def generate_spike_train(X,dt,theta,alpha, phi):
    spikes = numpy.zeros_like(X)
    for i,x in enumerate(X):
        if numpy.random.rand() < rate(x,theta,alpha,phi)*dt:
            spikes[i] = 1
    return spikes
...
if __name__=="__main__":
    ou = make_OU(100000,0.001,0.5,1.0)

    spike_1 = generate_spike_train(ou,0.001,0.8,1.0, 1.0)
    spike_times_1 = numpy.where(spike_1==1)
    spike_2 = generate_spike_train(ou,0.001,0.8,0.2, 1.0)
    ...
    ax4 = ppl.subplot2grid((2,4),[1,3],sharey=ax2, sharex=ax3)
    ...
    rates1 = rate(xs,0.5,1.0, 1.0)
    rates2 = rate(xs,0.5,0.2, 1.0)

    ppl.plot(rates1,xs,ax=ax3)
    ppl.plot(rates2,xs,ax=ax4)

    ppl.plt.gcf().suptitle('Precision and Frequency Trade-off in Poisson Processes')
